...
# load word2vec
m_sen_fix_len = 100 #句子长度 
e_sen_fix_len = 28 #句子长度 
tr_size = 1000 #
batch_size = 10 
epoch_size = 40
...
def gen_mention_text_vec():
    mention_loc_textVec_dict = {} 
    filename = './mention_context.txt'
    with open(filename) as fr:
        for line in fr:
            tokens = line.split('\t')
            mention_loc_textVec_dict[tokens[0]] = get_m_sentence_vector(tokens[1])
    return mention_loc_textVec_dict  

def create_mention_entity_pairs(mention_textVec_dict, entity_text_dict_vec):
    #pairs = []
    inputs1 = []
    inputs2 = []
    labels = []
    with open("./data/eng_gold.tab") as fr:
        ...
        for line in fr:
            ...
            #print cnt
            try:
                mention_text_vec = mention_textVec_dict[mention_loc]
            except:
                continue
...
def compute_accuracy(prediction,labels):
    ...
    a = labels[prediction.ravel() < 0.5].mean()
    b = labels[prediction.ravel() > 0.5].mean()
    print "a:" + str(a)
    print "b" + str(b)
    return a
...
t_start = time.time()
mention_textVec_dict = dict(gen_mention_text_vec())
...
t_start = time.time()
inputs1, inputs2,tr_labels = create_mention_entity_pairs(mention_textVec_dict, entity_text_dict_vec) 
print "create_mention_entity_pairs ok, use time%.3fs" % (time.time() - t_start)

del entity_text_dict_vec
del mention_textVec_dict
	...
	for epoch in range(epoch_size):
		...
		for i in range(total_batch):
			...
			avg_loss += loss_value
		        ...
		        print('batch %d  time: %f loss %0.5f acc %0.2f' %(i,duration,loss_value,tr_acc))
			avg_acc +=tr_acc*100
		duration = time.time() - start_time
