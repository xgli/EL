...
#tf Graph Input
images_L = tf.placeholder(tf.float32,shape=([batch_size,n_input]),name='L')
images_R = tf.placeholder(tf.float32,shape=([batch_size,n_input]),name='R')
m_sf = tf.placeholder(tf.float32, shape=([batch_size,50]),name='m_sf')
e_sf = tf.placeholder(tf.float32, shape=([batch_size,50]),name="e_sf")
labels = tf.placeholder(tf.float32,shape=([batch_size,1]),name='gt')
dropout_f = tf.placeholder("float")



def mclnet(x,_dropout,sf):
...
with tf.variable_scope("siamese") as scope:
	model1 = mclnet(images_L, dropout_f,m_sf)
	scope.reuse_variables()
	model2 = mclnet(images_R, dropout_f,e_sf)
	...
	for epoch in range(epoch_size):
		...
		for i in range(total_batch):
			...
			input1,input2,y =next_batch(s,e,tr_pairs,tr_labels)
                        ...
                        #print input1.shape
                        #print input2
                        msf = np.array([[1]*50]*batch_size)
                        esf = np.array([[1]*50]*batch_size)
			_,loss_value,prediction = sess.run([train_op,loss,predict],
                                      feed_dict={images_L:input1,images_R:input2 ,labels:y,dropout_f:drop_out,m_sf:msf,e_sf:esf})
