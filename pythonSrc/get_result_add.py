...
cmn_es_find_dict = {}
filter_for_research_file = open('../data/dict/cmn_add_pinyin.tab', 'r')#词表
fw = open('../data/result/cmn/cmn_add_res.tab', 'w')
        ...
        for candidation in decodejson["hits"]["hits"]:
            ...
            for tag in candidation['highlight']['snt']:
                ...
                if(not flag):
                    ...
                    res += '\t' + candidation['_id'] + ':' + str(begin) + '-' + str(end) + '\t' + line_list[2] + '\t' + line_list[3] +"\t"+ line_list[4]
...
pickle.dump(cmn_es_find_dict,file("cmn_add_es_found.pk","wb"))
pickle.dump(cmn_author_and_pinyin_found_dict, file('cmn_add_es_found_loc.pk', 'wb'))
#%%English
print 'English'
...
filter_for_research_file = open('../data/dict/eng_add.tab', 'r')
fw = open('../data/result/eng/eng_add_res.tab', 'w')
...
fw.close()
filter_for_research_file.close()
pickle.dump(eng_author_and_words_found_dict, file('eng_add_es_found_loc.pk', 'wb'))
pickle.dump(eng_es_find_dict,file("eng_add_es_found.pk","wb"))
...
filter_for_research_file = open('../data/dict/spa_add.tab', 'r')
fw = open('../data/result/spa/spa_add_res.tab', 'w')
...
fw.close()
filter_for_research_file.close()
pickle.dump(spa_author_and_words_found_dict, file('spa_add_es_found_loc.pk', 'wb'))
pickle.dump(spa_es_find_dict,file("spa_add_es_found.pk","wb"))
